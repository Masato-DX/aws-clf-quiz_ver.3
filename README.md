...
同一の学習アルゴリズムを共有する、資格試験対策クイズアプリを収めたリポジトリです。

| アプリ | 対象試験 | 収録数 | 場所 |
|---|---|---|---|
| AWS CLF-C02 対策クイズ | AWS Certified Cloud Practitioner (CLF-C02) | 354問 | リポジトリ直下 |
| ウェブ解析士 認定試験 対策クイズ | ウェブ解析士認定試験（初級） | 320問 | [`web-analyst/`](./web-analyst) |

いずれも **間違えた問題を自動追跡して復習に誘導する出題制御**を中核に持ちます。
仕様は `docs/requirements.md` と `docs/design.md`（各アプリ配下）に記載しています。

> GitHub Pages はリポジトリにつき 1 サイトのため、公開できるアプリは同時に 1 つです。
