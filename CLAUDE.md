...
本アプリを別試験向けに作り直す／リファクタリングする場合は、まず以下を読むこと。

| ドキュメント | 内容 |
|---|---|
| `docs/requirements.md` | 要件定義書。機能要件・学習アルゴリズム要件・移植時の差し替えポイント |
| `docs/design.md` | 詳細設計書。データ構造・アルゴリズム実装・既知の制約・再実装時の推奨構成 |
...
| `aws_clf_wrong_counts` | 問題 ID ごとの累積間違い回数（要注意問題の判定に使用。正解すると削除され通常プールへ復帰） |
